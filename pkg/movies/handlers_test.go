...
import (
  ...
  "github.com/paarig/goyagi/pkg/model"
...
func TestRetrieveHandler(t *testing.T) {
  ...
  t.Run("retrieves movies on success", func(tt *testing.T) {
    c, rr := newContext(tt, nil)
    c.SetParamNames("id")
    c.SetParamValues("1")
    ...
    var response model.movieerr = json.Unmarshal(rr.Body.Bytes(), &response)
    require.NoError(tt, err)
